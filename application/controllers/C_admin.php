...
class C_admin extends CI_Controller {
	...
	{
		$data["datas"]=$this->m_admin->readLayananKami($slug);
		$data["sidebar"]=$slug;
	...
	public function editLayananKami($slug, $id){
		...
		$wst = $this->m_admin->get_layanan_kami("where id='$id'");
		$data = array(
			"id"=> $wst[0]['id'], 
			"nama" => $wst[0]['nama'], 
			"foto" => $wst[0]['foto'], 
	...
	public function save_layanan_kami(){
		...
		if ( ! $this->upload->do_upload('berkas')){
			...
			$this->m_admin->set_data($data,'layanan_kami');
			$data = array('upload_data' => $this->upload->data());
			redirect('c_admin/layananKami/'.$slug);
		}
	}
	...
	public function edit_layanan_kami($id){
		...
		if (!empty($_FILES['berkas']['name'])) {
			if ( ! $this->upload->do_upload('berkas')){
				...
				$this->m_admin->update_data($data,$id,'layanan_kami');
				$data = array('upload_data' => $this->upload->data());
				redirect('c_admin/layananKami/'.$slug);
			}
